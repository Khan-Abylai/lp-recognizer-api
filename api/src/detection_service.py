...
class DetectionEngine(object):
    ...
    __ENGINE_NAME = './models/engine/detector.'
    def __init__(self, img_w=512, img_h=512, grid_size=16):
        ...
        if os.path.exists(DetectionEngine.__ENGINE_NAME):
            with open(DetectionEngine.__ENGINE_NAME, 'rb') as f, trt.Runtime(self.trt_logger) as runtime:
...
class DetectionEngine_UAE(object):
    __MODELS_PATH = "./models/detector_weights_uae.np"
    ...
    def __init__(self, img_w=512, img_h=512, grid_size=16):
        ...
        self.grid_h = self.img_h // grid_size
        self.trt_logger = trt.Logger(trt.Logger.WARNING)
        if os.path.exists("/opt/api/models/engine/detection.engine"):
